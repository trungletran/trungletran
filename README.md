# Hi 👋, I'm Trung Le

🎓 Fourth-year student at International University  
💻 Software Engineering | Java Developer  
📍 Vietnam

---

## 👨‍💻 About Me
- 4th-year university student majoring in Computer Science.
- Main programming language: **Java**.
- Experience with academic and small-scale software projects.
- Strong interest in **backend development**, **data structures**, and **system design**.

---

## 🛠️ Technical Skills

### Languages
- Java
- JavaScript
- SQL
- C / C++

### Frameworks & Tools
- Spring Boot
- Node.js
- MySQL
- MongoDB
- Git & GitHub
- VS Code
- MySQL Workbench

---

## 📌 Projects
- **E-commerce Website**  
  University project with product management, cart, and checkout features.

- **Course Management System**  
  Backend-focused system with course creation, lecture management, and search functionality.

- **2048 Game (Java Swing)**  
  Desktop game implemented using Java Swing.

---

## 📚 Currently Learning
- Data Structures & Algorithms
- Database Design
- Software Engineering Principles
- Backend System Development

---

## 🎯 Career Goals
- Strengthen backend engineering skills
- Gain real-world development experience
- Secure an internship or junior software developer role

---

## 📫 Contact Me
- GitHub: https://github.com/trungletran
- Email: trungtranle2004@gmail.com
- LinkedIn: https://www.linkedin.com/in/trung-tr%E1%BA%A7n-l%C3%AA-b56786300/
